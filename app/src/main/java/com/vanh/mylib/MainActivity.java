...
import com.vanh.mylib.rv.ViewHolder2;
...
public class MainActivity extends BaseActivity<ActivityMainBinding, EmptyViewModel> {
    ...
    protected void initView() {
        ...
        List<ItemData<ViewHolder2>> list = new ArrayList<>();
        list.add(new Item("Hello"));
        list.add(new Item("Hello 1"));
        list.add(new Item("Hello 2"));
        list.add(new Item("Hello 3"));

        rv.setAdapter(new Adapter(new ItemTemplate<ViewHolder2>() {
            @Override
            public ViewHolder2 inflate(LayoutInflater layoutInflater, ViewGroup parent, boolean attachTo) {
                return new ViewHolder2(ItemviewBinding.inflate(layoutInflater, parent, attachTo), (view, viewHolder) -> {
